function [P,Uinit,output] = cp_als_qr_svd(X,R,varargin)
%CP_ALS_QR_SVD Compute a CP decomposition of any type of tensor.
%
...
%
% Adapted to use a QR and SVD to solve the LS problems instead of normal
...
%%% Add a tolerance level for the SVD. %%%
params.addParameter('svdtol',1e-8,@isscalar);
...
%%% Changes for cp_als_qr_svd start here: %%%
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        ...
        if isa(Y,'ktensor')
            ...
            %%% Calculate updated factor matrix by the svd of R0' and Z. %%%
            tic; U{n} = double(Z) * pinv(R0',svd_tol); t = toc; t_back = t_back + t;
        ...
        Unew = bsxfun(@rdivide, U{n}, lambda');t = toc; t_lamb = t;
        U{n} = Unew;
    ...
    %%% Changes for cp_als_qr_svd end here. %%%
